...
type classification = pgdozorv1.LogEvent_LogClassification

const classUnspecified = pgdozorv1.LogEvent_LOG_CLASSIFICATION_UNSPECIFIED
...
func connectionRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_REJECTED,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_AUTHENTICATED,
			`^connection authenticated: (?:user|identity)="\w+" method=\w+ \(\w+:\d+\)`,
			"connection authenticated: "),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_CLIENT_FAILED_TO_CONNECT,
		...
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_LOST_OPEN_TX,
			"unexpected EOF on client connection with an open transaction"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_LOST,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_LOST,
			`^could not (?:receive data from|send data to) client: [\w ]+`,
			"could not receive data from client", "could not send data to client"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_TERMINATED,
			"terminating connection due to administrator command"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_OUT_OF_CONNECTIONS,
			"remaining connection slots are reserved for non-replication superuser connections",
			"sorry, too many clients already"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_TOO_MANY_CONNECTIONS_ROLE,
			`^too many connections for role "([^"]+)"`, "too many connections for role"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_TOO_MANY_CONNECTIONS_DATABASE,
			`^too many connections for database "([^"]+)"`, "too many connections for database"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COULD_NOT_ACCEPT_SSL_CONNECTION,
			`^could not accept SSL connection: [\w ]+`, "could not accept SSL connection: "),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_PROTOCOL_ERROR_UNSUPPORTED_VERSION,
			`^unsupported frontend protocol \d+\.\d+: server supports \d+\.\d+ to \d+\.\d+`,
			"unsupported frontend protocol"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_PROTOCOL_ERROR_INCOMPLETE_MESSAGE,
			"incomplete message from client"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_RECEIVED, "connection received: "),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_AUTHORIZED, "connection authorized: "),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CONNECTION_DISCONNECTED, "disconnection: "),
	}
}

func walRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_RESTARTPOINT_AT,
			`^recovery restart point at (\w+)/(\w+)`, "recovery restart point at"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_WAL_INVALID_RECORD_LENGTH,
			`^invalid record length at (\w+)/(\w+)(?:: wanted \d+, got \d+)?`, "invalid record length at "),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_WAL_REDO,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_WAL_ARCHIVE_COMMAND_FAILED,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_WAL_ARCHIVE_COMMAND_FAILED,
			...
			"archive command"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_WAL_BASE_BACKUP_COMPLETE,
...
func lockRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_LOCK_ACQUIRED,
		...
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_LOCK_DEADLOCK_DETECTED, "deadlock detected"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_LOCK_TIMEOUT,
...
func applyLockWait(parts []string, e *AnalyzedLogEvent) {
	switch parts[1] {
	case "still waiting":
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_LOCK_WAITING
	case "avoided deadlock":
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_LOCK_DEADLOCK_AVOIDED
	case "detected deadlock while waiting":
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_LOCK_DEADLOCK_DETECTED
	}
}
...
func checkpointRules() []rule {
	return []rule{
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CHECKPOINT_TOO_FREQUENT,
...
func applyCheckpointStarting(parts []string, e *AnalyzedLogEvent) {
	if parts[1] == "restartpoint" {
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_RESTARTPOINT_STARTING
	} else {
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_CHECKPOINT_STARTING
	}
}

func applyCheckpointComplete(parts []string, e *AnalyzedLogEvent) {
	if parts[1] == "restartpoint" {
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_RESTARTPOINT_COMPLETE
	} else {
		e.Classification = pgdozorv1.LogEvent_LOG_CLASSIFICATION_CHECKPOINT_COMPLETE
	}
}

func vacuumRules() []rule {
	return []rule{
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_AUTOVACUUM_CANCEL, "canceling autovacuum task"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SKIPPING_VACUUM_LOCK_NOT_AVAILABLE, "skipping vacuum of"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SKIPPING_ANALYZE_LOCK_NOT_AVAILABLE, "skipping analyze of"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_TXID_WRAPAROUND_WARNING,
			`^database (with OID (\d+)|"(.+?)") must be vacuumed within (\d+) transactions`, "database"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_TXID_WRAPAROUND_ERROR,
			`^database is not accepting commands to avoid wraparound data loss in database (with OID (\d+)|"(.+?)")`,
			"database is not accepting commands to avoid wraparound data loss in database"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_AUTOVACUUM_COMPLETED, autoVacuumPattern,
			"automatic vacuum of table", "automatic aggressive vacuum of table",
			"automatic aggressive vacuum to prevent wraparound of table"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_AUTOANALYZE_COMPLETED, autoAnalyzePattern,
			"automatic analyze of table"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_AUTOVACUUM_LAUNCHER_STARTED, "autovacuum launcher started"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_AUTOVACUUM_LAUNCHER_SHUTTING_DOWN,
...
func serverFailureRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_CRASHED,
			`^server process \(PID (\d+)\) was terminated by signal (6|11)(: [\w ]+)?`, "server process"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_OUT_OF_MEMORY,
			`^server process \(PID (\d+)\) was terminated by signal (9)(: [\w ]+)?`,
			"out of memory", "server process"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_OUT_OF_MEMORY, "out of memory"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_CRASHED,
			"terminating any other active server processes",
			"terminating connection because of crash of another server process",
			"all server processes terminated; reinitializing"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_INVALID_CHECKSUM,
		...
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_STATS_COLLECTOR_TIMEOUT,
...
func serverLifecycleRules() []rule {
	return []rule{
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_RELOAD,
			"received SIGHUP, reloading configuration files"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_RELOAD,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_RELOAD,
			...
			"configuration file"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_SHUTDOWN,
		...
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_START,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_START,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_START_RECOVERING,
...
func serverMiscRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_MISC,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_TEMP_FILE_CREATED,
			`^temporary file: path "(.+?)", size (\d+)`, "temporary file: path "),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SERVER_PROCESS_EXITED,
...
func standbyRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_RESTORED_WAL_FROM_ARCHIVE,
			`^restored log file "([^"]+)" from archive`, "restored log file"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_STARTED_STREAMING,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_STREAMING_INTERRUPTED,
			`^could not receive data from WAL stream: ([\w: ]+)`, "could not receive data from WAL stream"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_STOPPED_STREAMING,
			"terminating walreceiver process due to administrator command"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_CONSISTENT_RECOVERY_STATE,
			`^consistent recovery state reached at (\w+)/(\w+)`, "consistent recovery state reached at"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_STATEMENT_CANCELED,
			"canceling statement due to conflict with recovery"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STANDBY_INVALID_TIMELINE,
...
func constraintRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_UNIQUE_CONSTRAINT_VIOLATION,
			`^duplicate key value violates unique constraint "(.+)"`,
			"duplicate key value violates unique constraint"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_FOREIGN_KEY_CONSTRAINT_VIOLATION,
			`^insert or update on table "(.+?)" violates foreign key constraint "(.+?)"`,
			"insert or update on table"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_FOREIGN_KEY_CONSTRAINT_VIOLATION,
			`^update or delete on table "(.+?)" violates foreign key constraint "(.+?)" on table "(.+?)"`,
			"update or delete on table"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_NOT_NULL_CONSTRAINT_VIOLATION,
			`^null value in column "(.+?)" violates not-null constraint`, "null value in column"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CHECK_CONSTRAINT_VIOLATION,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_EXCLUSION_CONSTRAINT_VIOLATION,
...
func schemaErrorRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SYNTAX_ERROR,
			`^syntax error at (?:end of input|or near "(.+)")(?: at character \d+)?`, "syntax error at"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COLUMN_MISSING_FROM_GROUP_BY,
			`^column "([^"]+)" must appear in the GROUP BY clause or be used in an aggregate function`+
				`(?: at character \d+)?`,
			"column"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COLUMN_DOES_NOT_EXIST,
			`^column "([^"]+)" of relation "([^"]+)" does not exist(?: at character \d+)?`, "column"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COLUMN_DOES_NOT_EXIST,
			`^column (?:"[^"]+"|[\w.]+) does not exist(?: at character \d+)?`, "column"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COLUMN_REFERENCE_AMBIGUOUS,
			`^column reference "([^"]+)" is ambiguous(?: at character \d+)?`, "column"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_RELATION_DOES_NOT_EXIST,
			`^relation "([^"]+)" does not exist(?: at character \d+)?`, "relation"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_FUNCTION_DOES_NOT_EXIST,
			`^function ([^"]+) does not exist(?: at character \d+)?`, "function"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_OPERATOR_DOES_NOT_EXIST,
			`^operator does not exist: (\w+) ([`+regexp.QuoteMeta("+*/<>=~!@#%^&|`?-")+`]+) (\w+)`+
				`(?: at character \d+)?`,
			"operator does not exist: "),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_PERMISSION_DENIED,
...
func valueErrorRules() []rule {
	return []rule{
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INVALID_INPUT_SYNTAX,
			`^invalid input syntax for [\w ]+(?:: "([^"]+)")?(?: at character \d+)?`, "invalid input syntax for"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_VALUE_TOO_LONG_FOR_TYPE,
			`^value too long for type ([\w ()]+)`, "value too long for type"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INVALID_VALUE,
			`^invalid value "([^"]+)" for "([^"]+)"`, "invalid value"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_MALFORMED_ARRAY_LITERAL,
			`^malformed array literal: "(.+)"(?: at character \d+)?`, "malformed array literal"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_SUBQUERY_MISSING_ALIAS,
			`^subquery in FROM must have an alias(?: at character \d+)?`, "subquery in FROM must have an alias"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INSERT_TARGET_COLUMN_MISMATCH,
			`^INSERT has more expressions than target columns(?: at character \d+)?`,
			"INSERT has more expressions than target columns"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_ANY_ALL_REQUIRES_ARRAY,
			`^op ANY/ALL \(array\) requires array on right side(?: at character \d+)?`,
			"op ANY/ALL (array) requires array on right side"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COLUMN_CANNOT_BE_CAST,
			`^column "([^"]+)" cannot be cast to type "([^"]+)"`, "column"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_DIVISION_BY_ZERO, "division by zero"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_CANNOT_DROP,
			`^cannot drop ([^"]+) because other objects depend on it`, "cannot drop"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INTEGER_OUT_OF_RANGE, "integer out of range"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_PARAM_MISSING,
			`^there is no parameter \$\d+(?: at character \d+)?`, "there is no parameter $"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_NO_SUCH_SAVEPOINT, "no such savepoint"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_UNTERMINATED_QUOTED_STRING,
			`^unterminated quoted string(?: at or near "(.+?)")?(?: at character \d+)?`,
			"unterminated quoted string"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_UNTERMINATED_QUOTED_IDENTIFIER,
			`^unterminated quoted identifier(?: at or near "(.+?)")?(?: at character \d+)?`,
			"unterminated quoted identifier"),
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INVALID_BYTE_SEQUENCE,
			`^invalid byte sequence for encoding "([^"]+)": (.*)`, "invalid byte sequence for encoding"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INVALID_REGEXP, "invalid regular expression: "),
	}
}

func transactionErrorRules() []rule {
	return []rule{
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_TRANSACTION_IS_ABORTED,
			"current transaction is aborted, commands ignored until end of transaction block"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_ON_CONFLICT_NO_CONSTRAINT_MATCH,
			"there is no unique or exclusion constraint matching the ON CONFLICT specification"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_ON_CONFLICT_ROW_AFFECTED_TWICE,
			"ON CONFLICT DO UPDATE command cannot affect row a second time"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COULD_NOT_SERIALIZE_REPEATABLE_READ,
			"could not serialize access due to concurrent update"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_COULD_NOT_SERIALIZE_SERIALIZABLE,
			"could not serialize access due to read/write dependencies among transactions"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_INCONSISTENT_RANGE_BOUNDS,
...
func statementRules(samples *statementSampleExtractor) []rule {
	...
	return []rule{
		reRuleApply(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STATEMENT_AUTO_EXPLAIN,
			...
			"duration: "),
		{
			class: pgdozorv1.LogEvent_LOG_CLASSIFICATION_STATEMENT_DURATION,
		...
		reRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STATEMENT_LOG,
			`^(?:statement|(?:execute|execute fetch from) (?:[^:]+)(?:/(?:[^:]+))?): (.*)`,
			"statement: ", "execute "),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STATEMENT_CANCELED_USER,
			"canceling statement due to user request"),
		prefixRule(pgdozorv1.LogEvent_LOG_CLASSIFICATION_STATEMENT_CANCELED_TIMEOUT,
